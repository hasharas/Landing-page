...
const FAQ = () => {
  return (
    <div className='mx-auto'>
        <h2 className='text-[27px] text-[#6B3CC9] poppins-semibold mt-24 mb-10'>Frequently asked questions</h2>
...
const FAQCard = ({title, text}) => {
    ...
    return (
        <div className={`bg-[#FAF8FF] ${isActive?"text-[#6B3CC9] h-[150px] ":"text-black h-[80px]"}  transition-all duration-300 lg:w-[896px] w-full p-[24px] mx-auto flex flex-col gap-[13px]`}>
            <div className='flex justify-between items-center cursor-pointer' onClick={handleActive}>
                <h3 className="text-left">{title}</h3>
                {isActive?<FaMinus />:<FaPlus />}
            </div>
            {isActive?<p className='text-[#6F6C90] text-justify fadeIn'>{text}</p>:<></>}
        </div>
    )
}
